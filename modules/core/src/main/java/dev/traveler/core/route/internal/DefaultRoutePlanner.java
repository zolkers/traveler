...
import dev.traveler.core.route.api.RoutePlanner;
...
public final class DefaultRoutePlanner implements RoutePlanner {
    ...
    public DefaultRoutePlanner(RouteSearchSettings settings) {
        this(settings, RouteSearchComponents.standard());
    }

    public DefaultRoutePlanner(RouteSearchSettings settings, RouteSearchComponents components) {
        this.settings = Objects.requireNonNull(settings, "settings");
        this.components = Objects.requireNonNull(components, "components");
        this.segmentPlanner = new SegmentPlanner();
    }

    @Override
    public RoutePlan plan(WorldLayer world, BlockPosition start, RouteGoal goal) {
        return plan(search(world, start, goal));
    }

    public RoutePlan plan(RouteSearchResult result) {
        RouteSearchResult safeResult = Objects.requireNonNull(result, "result");
        return safeResult.route()
                .map(route -> new RoutePlan(segmentPlanner.segments(route)))
                .orElseGet(() -> new RoutePlan(List.of()));
    }
    ...
    private PathfinderResult<BlockPosition> findPath(
        ...
        PathfinderRequest<BlockPosition> request =
                new PathfinderRequest<>(graph, start, goal, DefaultRoutePlanner::distance);
    ...
    private PathfinderResult<SurfaceNode> findSurfacePath(
        ...
        return bestReachableProgressPath(worldLayer, start, goal, starts)
                .orElseGet(DefaultRoutePlanner::surfaceNotFound);
    }
    ...
    private static PathfinderResult<SurfaceNode> bestFoundSurfacePath(
        ...
        return results.stream()
                ...
                .min(DefaultRoutePlanner::comparePathCost)
                .orElseGet(DefaultRoutePlanner::surfaceNotFound);
    }
